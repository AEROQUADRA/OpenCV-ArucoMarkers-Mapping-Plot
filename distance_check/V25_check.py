...
def find_aruco_like_markers(image):
    ...
    aruco_params.adaptiveThreshConstant = 2  # Try lowering this value
    aruco_params.minMarkerPerimeterRate = 0.01  # Try lowering this value
    aruco_params.maxMarkerPerimeterRate = 1.0  # Try increasing this value
    aruco_params.polygonalApproxAccuracyRate = 0.05  # Try lowering this value
    aruco_params.minCornerDistanceRate = 0.05  # Try lowering this value
    aruco_params.minDistanceToBorder = 2  # Try lowering this value
    aruco_params.minOtsuStdDev = 2.0  # Try lowering this value
    ...
    if ids is not None:
        ...
        # Existing contour processing
        aruco_like_contours = []
        for c in contours:
            perimeter = cv2.arcLength(c, True)
            approx = cv2.approxPolyDP(c, 0.04 * perimeter, True)
            if len(approx) == 4:
                aruco_like_contours.append(approx.reshape(-1, 2))

        aruco_like_contours = [
            c for c in aruco_like_contours if cv2.contourArea(c) > 100]

    # Rest of the existing code for processing contours
    if len(aruco_like_contours) >= 4:
        ...
        for img_corner in image_corners:
            distances = [np.linalg.norm(compute_centroid(
                c) - img_corner) for c in aruco_like_contours]
            closest_contour = aruco_like_contours[np.argmin(distances)]
            sorted_corner_pts = sort_corners(closest_contour)
            closest_contours.append({
                'centroid': compute_centroid(closest_contour),
        ...
        return coordinates, closest_contours
    else:
        return None

# Helper functions used in the find_aruco_like_markers function
...
def main():
    ...
    if corners_coordinates and len(corners_coordinates) == 4:
        # Draw circles on the original image at the contour points
        for contour in closest_contours:
            for point in contour['corners']:
                cv2.circle(original_image, tuple(int(x) for x in point),
